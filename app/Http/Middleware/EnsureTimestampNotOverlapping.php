...
class EnsureTimestampNotOverlapping
{
    public function handle(Request $request, Closure $next): Response {
        ...
        if ($checkIn && $this->isCheckInTimeOverlapping($checkInTime, $currTs, $prevTs)) {
            $prevMsg = $prevTs ? " or smaller than previous check out $prevTs->check_out" : "";
    ...
    private function isCheckInTimeOverlapping(string $checkInTime, Timestamp $currTs, Timestamp $prevTs = null): bool {
        $checkInTime = date_create_immutable($checkInTime);
        $currCheckOutDate = date_create_immutable($currTs->check_out);

        if (!$prevTs) {
            return $checkInTime >= $currCheckOutDate;
        }

        $prevCheckOutDate = date_create_immutable($prevTs->check_out);
    ...
    private function isCheckOutTimeOverlapping(string $checkOutTime, Timestamp $currTs, Timestamp $nextTs = null): bool {
        $currCheckInDate = date_create_immutable($currTs->check_in);
        $checkOutTime = date_create_immutable($checkOutTime);

        if (!$nextTs) {
            return $checkOutTime <= $currCheckInDate;
        }

        $nextCheckInDate = date_create_immutable($nextTs->check_in);
